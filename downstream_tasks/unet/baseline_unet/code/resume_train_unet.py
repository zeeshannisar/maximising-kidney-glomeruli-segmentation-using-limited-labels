...
def resumetrainunet(number_of_classes, class_weights, epochs, output_path, label, initialepoch=None, val_loss=None):
    ...
    if config['detector.reducelr']:
        if initial_epoch >= config['detector.reduceLR_patience'] and K.get_value(UNet.optimizer.learning_rate) > 0.00001:
    ...
    if config['detector.validationinputpath'] is not None:
        ...
        UNet.fit(train_flow, initial_epoch=initialepoch, epochs=(epochs - 100), shuffle=True, callbacks=callbacklist)#, use_multiprocessing=True, verbose=2, workers=8, max_queue_size=650)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Resume training baseline unet model.')
    ...
    resumetrainunet(number_of_classes, class_weights, config['detector.epochs'], config['detector.outputpath'], initialepoch=args.epoch, label=args.label, val_loss=args.valloss)
