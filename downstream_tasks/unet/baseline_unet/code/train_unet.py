...
def trainunet(config, number_of_classes, class_weights, label=None):
    ...
    #                                      config['detector.segmentationmodel'] + label + "_{epoch:02d}", ".hdf5")
    model_filename_latest = os.path.join(config['detector.outputpath'], "models",
                                         f"{config['detector.segmentationmodel']}_latest." + label + ".keras")

    if config['detector.validationinputpath'] is not None:
        callbacklist.append(tf.keras.callbacks.ModelCheckpoint(model_filename, monitor='val_loss', verbose=1,
                                                               save_best_only=True, mode='auto'))
    else:
        callbacklist.append(tf.keras.callbacks.ModelCheckpoint(model_filename, monitor='loss', verbose=1,
                                                               save_best_only=True, mode='auto'))

    callbacklist.append(tf.keras.callbacks.ModelCheckpoint(model_filename_latest, verbose=1,
                                                           save_freq=int(1 * len(train_flow))))
    ...
    if config['detector.earlyStopping']:
        callbacklist.append(tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=10, verbose=1, mode='auto'))

    callbacklist.append(DisplayLearningRateValue())

    if config['detector.reducelr']:
        # callbacklist.append(tf.keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=5,
...
def derived_parameters(conf, arguments):
    ...
    if conf['detector.transferlearning']:
        ...
        if 'simclr' in conf['transferlearning.pretrained_ssl_model']:
            ...
            raise ValueError("Self-supervised learning based pretrained-models should be one of ['simclr', 'byol', "
                             "'csco']")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Train baseline unet model.')

    parser.add_argument('-c', '--configfile', type=str, default='configuration_files/baseline/hubmap.cfg')
    parser.add_argument('-l', '--label', type=str, help='saved model name', default='baseline_unet_hubmap')
    ...
    parser.add_argument('-pm', '--pretrained_ssl_model', type=str, default='None')
    ...
    parser.add_argument('-pn', '--percent_N', type=str, default='percent_100', help='percent_1 | percent_5 | '
