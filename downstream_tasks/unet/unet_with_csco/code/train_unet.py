...
def images_with_labels_plot(images_H, images_O, mask, output_dir=None, name=None):
    rows = 5
    cols = 3
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            if c == 1:
                ...
                ax.axis("off")
            if c == 2:
                ax.imshow(mask[r, :, :, 0], cmap="gray")
                ax.set_title("mask", fontdict={'fontsize': font_size, 'fontweight': 'medium'})
                ax.axis("off")
            cnt += 1

    figure.tight_layout()
    figure.savefig(os.path.join(output_dir, name), bbox_inches="tight")
    plt.close(figure)
...
def trainunet(config, number_of_classes, class_weights, label=None):
    ...
    if config['detector.transferlearning']:
        ...
        UNet, inp_shape, otp_shape = unet_models.build_UNetAboveCSCO(config['transferlearning.csco_model_path'],
                                                                     config['transferlearning.pretrained_model_trainable'],
                                                                     input_shape, number_of_classes,
                                                                     config['detector.network_depth'],
                                                                     config['detector.filter_factor_offset'],
                                                                     config['detector.weightinit'],
                                                                     config['detector.padding'],
                                                                     config['detector.modifiedarch'],
                                                                     config['detector.batchnormalisation'],
                                                                     config['detector.kernel_size'],
                                                                     config['detector.dropout'],
                                                                     config['detector.learnupscale'])
    else:
        pretrained_model = None
        UNet, inp_shape, otp_shape = unet_models.baseline_unet_model(input_shape, number_of_classes,
    ...
    if config['detector.validationinputpath'] is not None:
        UNet.fit(train_flow, epochs=config['detector.epochs'], shuffle=True, callbacks=callbacklist,
                 validation_data=valid_flow, verbose=2)#, use_multiprocessing=True, workers=8, max_queue_size=650)
    else:
        UNet.fit(train_flow, epochs=config['detector.epochs'], shuffle=True, callbacks=callbacklist,
                 verbose=2)#, use_multiprocessing=True, workers=8, max_queue_size=650)
