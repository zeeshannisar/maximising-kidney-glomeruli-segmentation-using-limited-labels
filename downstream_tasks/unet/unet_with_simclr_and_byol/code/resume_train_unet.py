...
def resumetrainunet(number_of_classes, class_weights, epochs, output_path, label, initialepoch=None, val_loss=None):
    ...
    if config['detector.LR_weightdecay'] is not None:
       # import tensorflow_addons as tfa
        ...
        if config['detector.transferlearning'] and config['transferlearning.pretrained_ssl_model'] == 'byol':
    ...
    UNet = load_model(model_filename_latest, custom_objects=custom_objects)
