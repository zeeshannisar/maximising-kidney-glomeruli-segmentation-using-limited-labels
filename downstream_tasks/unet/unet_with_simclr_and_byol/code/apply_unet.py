    ...
    for i in range(0, imgpatch.shape[0] - (inp_patch_size[0] - 1), stride):
        for j in range(0, imgpatch.shape[1] - (inp_patch_size[1] - 1), stride):
                    ...
                    for subdir in sorted(
                        ...
                        if subdir != 'background':
                            files += [os.path.join(data_dir, target_stain, 'downsampledpatches', 'train', 'images',
                                                   subdir, name) for name in os.listdir(os.path.join(data_dir,
                                                                                                     target_stain,
                                                                                                     'downsampledpatches',
                                                                                                     'train', 'images',
                                                                                                     subdir))]
            ...
            if config['transferlearning.finetune']:
                if 'csco' in config['transferlearning.pretrained_ssl_model']:
                    patch_H, patch_O = stain_transform.separate(patch, staincode=config["general.staincode"])
                    # print(f"separating patch channels for stain: {config['general.staincode']} done..!")

            if standardise_patches:
                if config['transferlearning.finetune']:
                    if 'csco' in config['transferlearning.pretrained_ssl_model']:
                        patch_H = data_utils.standardise_sample(patch_H)
                        patch_O = data_utils.standardise_sample(patch_O)
                        # print("Standardising patch channels done..!")
                    else:
            ...
            if normalise_patches:
                if config['transferlearning.finetune']:
                    if 'csco' in config['transferlearning.pretrained_ssl_model']:
                        patch_H = data_utils.normalise_sample(patch_H, mean, stddev)
                        patch_O = data_utils.normalise_sample(patch_O, mean, stddev)
                        # print("Normalising patch channels done..!")
                    else:
            ...
            if c == batch_size:
                ...
                if config['transferlearning.finetune']:
                    if 'csco' in config['transferlearning.pretrained_ssl_model']:
                        ...
                        # print("Predicting patch channels done for csco...!")
                    else:
                        preds = UNet.predict(patch_batch)
                        # print("Predicting patch channels done for simclr, byol...!")
                else:
                    preds = UNet.predict(patch_batch)
    ...
    for file in imageList:
        ...
        with tempfile.NamedTemporaryFile(dir=tmpDir, suffix='.h5') as tmp_h5, \
            ...
            segmentation = h5pyfile.create_dataset("segmentation", (
                imagesize[0] + padding_y, imagesize[1] + padding_x, number_of_classes),
            ...
            for y in range(0, imagesize[0], PATCH_SIZE[0] - inp_patch_size[0] + stride):
                for x in range(0, imagesize[1], PATCH_SIZE[1] - inp_patch_size[1] + stride):
                    ...
                    if not first_patch:
                        ...
                        if config['transferlearning.finetune']:
                            if 'csco' in config['transferlearning.pretrained_ssl_model']:
                                segmentation, segmentation_count, patch_batch, patch_batch_H, patch_batch_O, patch_coords, \
                                    t, c, first_run = _process_patch(UNet, diff, imgpatch, inp_patch_size, otp_patch_size,
                                                                    stride, batch_size, patch_batch, patch_coords, patchx,
                                                                    patchy, standardise_patches,
                                                                    config['normalisation.normalise_patches'], mean,
                                                                    stddev, t, c, segmentation, segmentation_count,
                                                                    first_run, config['segmentation.stain_transfer'],
                                                                    threshold, config['general.datapath'],
                                                                    patch_batch_H=patch_batch_H,
                                                                    patch_batch_O=patch_batch_O)
                            else:
                                segmentation, segmentation_count, patch_batch, patch_coords, \
                                    t, c, first_run = _process_patch(UNet, diff, imgpatch, inp_patch_size, otp_patch_size,
                                                                    stride, batch_size, patch_batch, patch_coords, patchx,
                                                                    patchy, standardise_patches,
                                                                    config['normalisation.normalise_patches'], mean,
            ...
            if not first_patch:
                ...
                if config['transferlearning.finetune']:
                    if 'csco' in config['transferlearning.pretrained_ssl_model']:
                        segmentation, segmentation_count, patch_batch, patch_batch_H, patch_batch_O, patch_coords, \
                            t, c, first_run =_process_patch(UNet, diff, imgpatch, inp_patch_size, otp_patch_size, stride,
                                                            batch_size, patch_batch, patch_coords, patchx, patchy,
                                                            standardise_patches, config['normalisation.normalise_patches'],
                                                            mean, stddev, t, c, segmentation, segmentation_count, first_run,
                                                            config['segmentation.stain_transfer'], threshold,
                        ...
                        segmentation, segmentation_count, patch_batch, patch_coords, \
                            t, c, first_run = _process_patch(UNet, diff, imgpatch, inp_patch_size, otp_patch_size, stride,
                                                            batch_size, patch_batch, patch_coords, patchx, patchy,
                                                            standardise_patches, config['normalisation.normalise_patches'],
                                                            mean, stddev, t, c, segmentation, segmentation_count,
                                                            first_run, config['segmentation.stain_transfer'], threshold,
                                                            config['general.datapath'], patch_batch_H=None,
                                                            patch_batch_O=None)
                else:
                    segmentation, segmentation_count, patch_batch, patch_coords, \
                        t, c, first_run = _process_patch(UNet, diff, imgpatch, inp_patch_size, otp_patch_size, stride,
            ...
            #    preds = t.prediction
            #    prev_patch_coords = t.patch_coords
            #    for i, coord in enumerate(prev_patch_coords):
