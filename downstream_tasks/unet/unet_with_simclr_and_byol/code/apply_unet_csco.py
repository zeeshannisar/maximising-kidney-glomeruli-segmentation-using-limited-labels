...
def apply_model_from_disk(tmpDir, imageList, config, modelfilename, modeltimestamp, normalisation_filename):
    ...
    print(os.path.join(modelpath, 'models', modelfilename))
                        ...
                        if 'csco' in config['transferlearning.pretrained_ssl_model']:
                            segmentation, segmentation_count, patch_batch, patch_batch_H, patch_batch_O, \
                                patch_coords, t, c, first_run = _process_patch(UNet, diff, imgpatch, inp_patch_size,
                                                                               ...
                                                                               standardise_patches,
                                                                               config[
                                                                                   'normalisation.normalise_patches'],
            ...
            if not first_patch:
                ...
                if 'csco' in config['transferlearning.pretrained_ssl_model']:
                    segmentation, segmentation_count, patch_batch, patch_batch_H, patch_batch_O, \
                        patch_coords, t, c, first_run = _process_patch(UNet, diff, imgpatch, inp_patch_size,
                                                                       otp_patch_size,
                                                                       stride, batch_size, patch_batch, patch_coords,
                                                                       patchx,
...
def pretrained_csco_model_path(conf):
    return os.path.join(conf['general.homepath'], 'code/improve_kidney_glomeruli_segmentation/pre_training',
                        'models/CSCO/csco_unet_encoder_{}.hdf5'.format(conf['general.staincode']))
...
if __name__ == '__main__':
    ...
    print(os.path.join(config['detector.outputpath'],  'graphs', 'loss_history.' + label + '.png'))
